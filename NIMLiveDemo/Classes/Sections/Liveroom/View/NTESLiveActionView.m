...
- (UIButton *)pkButton {
    if (!_pkButton) {
        ...
        _pkButton.tag = NTESLiveActionTypePk;
        [_pkButton setImage:[UIImage imageNamed:@"icon_pk_n"] forState:UIControlStateNormal];
        [_pkButton setImage:[UIImage imageNamed:@"icon_pk_p"] forState:UIControlStateHighlighted];
        [_pkButton addTarget:self action:@selector(onAction:) forControlEvents:UIControlEventTouchUpInside];
        _pkButton.size = [UIImage imageNamed:@"icon_camera_zoom_n"].size;
    }
    return _pkButton;
}
