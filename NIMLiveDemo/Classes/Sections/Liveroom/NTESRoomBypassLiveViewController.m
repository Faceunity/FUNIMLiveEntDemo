//
//  NTESRoomBypassLiveViewController.m
//  NIMLiveDemo
//
...
@interface NTESRoomBypassLiveViewController ()<NIMChatroomManagerDelegate,NTESLiveInnerViewDelegate,NTESLiveAnchorHandlerDelegate,
...
@property (nonatomic, strong) NSString *peerPushUrl;

@property (nonatomic, strong) NSString *peerLayoutParam;

@property (nonatomic, assign) BOOL exitRoom;

@end

@implementation NTESRoomBypassLiveViewController
...
- (void)dealloc{
    ...
    if (!_exitRoom || [NTESLiveManager sharedInstance].type != NTESLiveTypeVideo) {
...
- (void)updateExitButtonStatus
{
    NSMutableArray *currentUids = [[NTESLiveManager sharedInstance] uidsOfConnectorsOnMic];
    BOOL exitButtonShouldHidden = (currentUids.count == 0);
    [self.innerView updateExitButtonHidden:exitButtonShouldHidden];
}
...
- (void)onActionType:(NTESLiveActionType)type sender:(id)sender
{
    ...
    switch (type) {
        case NTESLiveActionTypeLive:{
            if (!self.capture.isLiveStream || _exitRoom) {
...
- (void)onExitRoom
{
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"确定离开直播吗？" delegate:nil cancelButtonTitle:@"取消" otherButtonTitles:@"离开", nil];
...
- (void)doExitRoom
{
    ...
    if ([NTESLiveManager sharedInstance].type == NTESLiveTypeAudio) {
        [self.innerView switchToAudioTypeExitView];
        self.capture.canContinueLiveStream = YES;
    }
    else
    {
        if (self.delegate && [self.delegate respondsToSelector:@selector(onExitRoom)]) {
...
- (void)sendAnchorJoined
{
    NIMMessage *message = [NTESSessionMsgConverter msgWithAnchorJoined];
    NIMSession *session = [NIMSession session:self.chatroom.roomId type:NIMSessionTypeChatroom];
    [[NIMSDK sharedSDK].chatManager sendMessage:message toSession:session error:nil];
}
...
- (void)sendAnchorLeft
{
    NIMMessage *message = [NTESSessionMsgConverter msgWithAnchorLeft];
    NIMSession *session = [NIMSession session:self.chatroom.roomId type:NIMSessionTypeChatroom];
    [[NIMSDK sharedSDK].chatManager sendMessage:message toSession:session error:nil];
}
...
- (void)doExitLive
{
    ...
    [[NIMSDK sharedSDK].chatroomManager updateChatroomInfo:request completion:^(NSError * _Nullable error) {
        DDLogInfo(@"updateChatroomInfo error %@",error);
    }];
...
- (NSArray *)bypassTaskConfig
{
    NIMNetCallBypassTaskConfig *myConfig = [[NIMNetCallBypassTaskConfig alloc] init];
    
    NIMNetCallBypassTaskConfig *peerConfig = [[NIMNetCallBypassTaskConfig alloc] init];

    if (_currentMeeting.option.bypassTaskConfig.count) {
        myConfig = [_currentMeeting.option.bypassTaskConfig objectAtIndex:0];
    }
    myConfig.streamingLayoutMode = NIMNetCallBypassStreamingMixModeCustomVideoLayout;
    myConfig.streaminglayoutConfig = [NTESUserUtil bypassStreamingMixCustomLayoutConfigForPK];
    NTESMicConnector *connector = [NTESLiveManager sharedInstance].dstPkAnchor;

    peerConfig.taskId = [NSString stringWithFormat:@"%@%@",connector.uid,[NTESLiveUtil currentTimeStr]];
    
    peerConfig.streamingUrl = self.peerPushUrl;
    peerConfig.streamingLayoutMode = NIMNetCallBypassStreamingMixModeCustomVideoLayout;
    peerConfig.streaminglayoutConfig = self.peerLayoutParam;
    peerConfig.accid = connector.uid;
    NSArray *taskArray = @[myConfig,peerConfig];
...
- (void)doReserveAndJoinWithAnchorNewPkMeeting
{
    ...
    option.bypassTaskConfig = [self bypassTaskConfig];
...
- (void)doSendOnlineRequestToUser:(NSString *)dstUserId roomId:(NSString *)roomId {
    ...
    NIMCustomSystemNotification *notification = [NTESSessionCustomNotificationConverter notificationWithPkRoomBypassOnlineRequest:_chatroom.roomId];
    
    __weak typeof(self) weakSelf = self;
    NIMChatroomMember *member = [[NTESLiveManager sharedInstance] myInfo:_chatroom.roomId];
    DDLogInfo(@"YAT 发送 room bypass PkOnlineRequest：[%@] -> [%@]", member.userId, dstUserId);
...
- (void)doSendPkRequestToUser:(NSString *)dstUserId {
    NIMSession *session = [NIMSession session:dstUserId type:NIMSessionTypeP2P];
    NIMCustomSystemNotification *notification = [NTESSessionCustomNotificationConverter notificationWithPkRequest:_chatroom.roomId pushUrl:self.chatroom.broadcastUrl layoutParam:[NTESUserUtil bypassStreamingMixCustomLayoutConfigForPK]];
...
- (void)didReceivePkRequest:(NSString *)pushUrl layoutParam:(NSString *)layoutParam{
    ...
    self.peerPushUrl = pushUrl;
    self.peerLayoutParam = layoutParam;
